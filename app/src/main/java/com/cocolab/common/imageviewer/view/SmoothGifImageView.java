package com.cocolab.common.imageviewer.view;
 ...
 * 
 */
public class SmoothGifImageView extends GifImageView {
	...
	private TransformListener mInTransformListener;
	private TransformListener mOutTransformListener;

	public SmoothGifImageView(Context context) {
		super(context);
		initView();
	}

	public SmoothGifImageView(Context context, AttributeSet attrs) {
		super(context, attrs);
		initView();
	}

	public SmoothGifImageView(Context context, AttributeSet attrs, int defStyle) {
...
//		setBackgroundColor(mBgColor);
	 ...
	 */
	public void transformIn(TransformListener listener) {
		if(listener != null){
			mInTransformListener = listener;
		}
	 ...
	 */
	public void transformOut(TransformListener listener) {
		if(listener != null){
			mOutTransformListener = listener;
		}
	...
	private void initTransform() {
		...
		if (mBitmap == null || mBitmap.isRecycled()) {
			...
			if(drawable instanceof GifDrawable) {
				mBitmap = ((GifDrawable) drawable).getCurrentFrame();
	...
	private void startTransform(final int state) {
		...
		if (state == STATE_TRANSFORM_IN) {
			...
			PropertyValuesHolder alphaHolder = PropertyValuesHolder.ofInt("alpha", 0/*255*/, 0);
			valueAnimator.setValues(scaleHolder, leftHolder, topHolder, widthHolder, heightHolder, alphaHolder);
		}
		...
		valueAnimator.addListener(new ValueAnimator.AnimatorListener() {
			...
			public void onAnimationEnd(Animator animation) {
				...
				if (state == STATE_TRANSFORM_IN) {
					//进场动画播放完成
					if(mInTransformListener != null){
						mInTransformListener.onTransformComplete(state);
						mInTransformListener = null;//清除监听
					}
					mState = STATE_NORMAL;
				}else if (state == STATE_TRANSFORM_OUT) {
					//出场动画播放完成
					if(mOutTransformListener != null){
						mOutTransformListener.onTransformComplete(state);
						mOutTransformListener = null;//清除监听
					}
					//mState = STATE_NORMAL;
				}
